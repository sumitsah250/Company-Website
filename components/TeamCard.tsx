...
export default function TeamCard({ name, title, initials, color, accentColor = "from-white/10 to-white/5" }: TeamCardProps) {
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-surface-elevated border border-border/40 hover:border-white/20 transition-all duration-700">
      {/* Image area */}
      <div className="relative aspect-[3/4] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div 
            className={`w-28 h-28 rounded-full bg-gradient-to-br ${accentColor} border-2 border-white/10 flex items-center justify-center backdrop-blur-sm transition-all duration-700 group-hover:scale-110 group-hover:border-white/20 group-hover:shadow-lg group-hover:shadow-white/5`}
          >
      ...
      {/* Bottom glow */}
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700\" />
    </div>
  );
}
