        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <ScrollReveal direction="up" distance={30}>
              <div>
                <span className="text-sm font-medium text-[#211F1B] uppercase tracking-[0.15em] mb-6 block">Products</span>
            ...
            {products.map((product, i) => (
              <ScrollReveal key={product.name} direction="up" distance={25} delay={i * 0.1}>
                <TiltCard intensity={6}>
                  <div className="group relative p-8 sm:p-10 rounded-2xl bg-[#F4EFE6] border border-border/40 hover:border-border/60 transition-all duration-700 h-full glow-subtle hover:glow-subtle">
                    <div className="absolute top-6 right-6">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${
                        product.status === "Available"
                          ? "bg-[#EDE6DA] text-[#211F1B] border-[#C8BAA9]"
                          : product.status === "In Development"
                          ? "bg-[#E5DCCF] text-[#211F1B] border-[#C8BAA9]"
                          : "bg-[#FFFDF8] text-[#211F1B] border-[#D8CEC0]"
                      }`}>
                        {product.status}
                      </span>
                    </div>
                    ...
                    <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${product.color} flex items-center justify-center mb-6`}>
                      <product.icon className="w-7 h-7 text-[#211F1B]" />
                    </div>

                    <h3 className="text-2xl font-bold mb-2 text-[#211F1B]">{product.name}</h3>
                    <p className="text-[#211F1B] text-sm font-medium mb-4">{product.tagline}</p>
                    <p className="text-[#211F1B] leading-[1.8] mb-6">{product.description}</p>

                    <ul className="space-y-2 mb-8">
                      {product.features.map((feature) => (
                        <li key={feature} className="flex items-center gap-2 text-sm text-[#211F1B]">
                          <div className="w-1.5 h-1.5 rounded-full bg-[#211F1B]/40" />
                          {feature}
                        </li>
                      ))}
                    </ul>
                    ...
                    <button
                      className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold transition-all duration-300 ${
                        product.status === "Available"
                          ? "bg-[#181612] text-[#F7F3EB] hover:bg-[#2A2721] hover:shadow-[0_10px_25px_rgba(24,22,18,0.12)] hover:scale-[1.02]"
                          : "bg-[#F3EEE7] text-[#211F1B] border border-[#D8CEC0] cursor-not-allowed"
                      }`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal direction="scale" distance={0}>
            <div className="max-w-3xl mx-auto text-center">
              ...
              <p className="text-[#211F1B] leading-[1.8] mb-8">
              ...
              <Link href="/contact" className="group relative inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-[#181612] text-[#F7F3EB] font-semibold overflow-hidden transition-all duration-300 hover:bg-[#2A2721] hover:shadow-[0_10px_25px_rgba(24,22,18,0.14)] hover:scale-[1.02]">
